...
function AdminPendingRequest() {
  ...
  const {
    ...
    pendingManualFundingLoading,
    pendingManualFundingError,
    isReviewingManualFunding,
    manualFundingReviewingId,
    manualFundingAll,
    manualFundingAllLoading,
    manualFundingAllError,
    manualFundingAllPage,
    manualFundingAllTotalPages
  ...
  return (
    <div className='pl-4 space-y-6 text-slate-800 dark:text-slate-100'>
      <CustomTab TABS={TABS} activeTab={activeTab} setActiveTab={setActiveTab}>
        {activeTab === 'pending' && (
          <div className='space-y-4'>
            ...
            {!pendingManualFundingLoading && pendingManualFunding.length === 0 && !pendingManualFundingError && <div className='text-xs italic text-slate-500 dark:text-slate-400'>No pending requests.</div>}
            {pendingManualFunding.length > 0 && (
              <div className='border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 rounded-xl overflow-x-auto'>
                <table className='w-full text-xs min-w-[760px]'>
                  ...
                  <tbody>
                    {pendingManualFunding.map((item) => {
                      ...
                      return (
                        <tr key={item.id || item.transactionId || item.createdDate} className='border-t border-slate-200 dark:border-slate-700'>
                          ...
                          <td className='p-2'>
                            <div className='flex gap-2'>
                              <Button variant='primary' disabled={rowLoading} onClick={() => adminService.reviewManualFunding(item.id, 'approve', dispatch)} className='text-xs px-2 py-1'>
                                {rowLoading ? '...' : 'Approve'}
                              </Button>
                              <Button variant='secondary' disabled={rowLoading} onClick={() => adminService.reviewManualFunding(item.id, 'reject', dispatch)} className='text-xs px-2 py-1 !bg-red-500 !text-white hover:!bg-red-600'>
                                {rowLoading ? '...' : 'Reject'}
                              </Button>
                            </div>
                          </td>
          ...
          <div className='space-y-4'>
            <div className='grid md:grid-cols-3 gap-4 md:items-center'>
              <div className='md:col-span-2 flex gap-3'>
                <div>
                  <p className='text-xs font-semibold text-slate-600 dark:text-slate-300'>Search</p>
                  <input value={allFilters.search} onChange={(e) => setAllFilters((f) => ({ ...f, search: e.target.value, page: 1 }))} className='border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 rounded-sm px-2 py-1 text-xs' placeholder='Name/Email/Ref' />
                </div>
                <div>
                  <p className='text-xs font-semibold text-slate-600 dark:text-slate-300'>Currency</p>
                  <select value={allFilters.currency} onChange={(e) => setAllFilters((f) => ({ ...f, currency: e.target.value, page: 1 }))} className='border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 rounded-sm px-2 py-1 text-xs'>
                ...
                <div>
                  ...
                  <select value={allFilters.status} onChange={(e) => setAllFilters((f) => ({ ...f, status: e.target.value, page: 1 }))} className='border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 rounded-sm px-2 py-1 text-xs'>
              ...
              <div className='col-span-1 flex gap-2 flex-wrap'>
                ...
                <div className='w-fit'>
                  <Button variant='secondary' onClick={onAllRefresh} className='text-xs px-2 py-1'>Refresh</Button>
                </div>
              </div>
            </div>
            ...
            {!manualFundingAllLoading && manualFundingAll.length === 0 && !manualFundingAllError && <div className='text-xs italic text-slate-500 dark:text-slate-400'>No records.</div>}
                      ...
                      return (
                        <tr key={item.id || item.transactionId || item.createdDate} className='border-t border-slate-200 dark:border-slate-700'>
                          ...
                          <td className='p-2'>
                            ...
                            ) : <span className='text-xs italic text-slate-500 dark:text-slate-400'>-</span>}
                          </td>
                        </tr>
                      );
                    })}
            ...
            <div className='flex flex-col gap-2 text-xs'>
              <div className='flex flex-wrap items-center justify-between gap-4'>
                <div className='flex gap-3'>
                  ...
                  <select value={allFilters.limit} onChange={(e) => setAllFilters((f) => ({ ...f, limit: Number(e.target.value), page: 1 }))} className='border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 rounded-sm px-2 py-1 text-xs'>
                    {[5, 10, 20, 50].map((l) => <option key={l} value={l}>{l}</option>)}
                  </select>
                </div>
                <div className='flex gap-2'>
                  <Button variant='secondary' disabled={allFilters.page <= 1} onClick={() => setAllFilters((f) => ({ ...f, page: f.page - 1 }))} className='text-xs px-3 py-1'>Prev</Button>
                  <Button variant='secondary' disabled={allFilters.page >= manualFundingAllTotalPages} onClick={() => setAllFilters((f) => ({ ...f, page: f.page + 1 }))} className='text-xs px-3 py-1'>Next</Button>
                </div>
              </div>
            </div>
          </div>
