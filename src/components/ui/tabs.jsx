...
export function TabsList({ children, tabClassName }) {
  return <div className={`flex justify-between border-b border-gray-200 ${tabClassName}`}>{children}</div>
}
      ...
      className={`flex-1 text-center px-4 py-2 text-sm border-b-2 transition-colors
        ${
          isActive
            ...
            : `border-transparent text-primary hover:text-gray-700 hover:border-gray-300 ${inactiveClassName}`
        }`}
    >
      {children}
    </button>
